...
def train_model_binary(
    *,
    ...
    """Same shape as `train_model` but for the 2-class directional classifier.

    Labels must be strictly {-1, +1}; drop the flat rows BEFORE calling this
    (see `triple_barrier_labels_directional`). The reproducibility hash
    space is disjoint from the 3-class pipeline's because model.params
    differ (`objective=binary` vs `objective=multiclass`).
    """
    if not 0.0 < calibration_fraction < 1.0:
        raise ValueError("calibration_fraction must be in (0.0, 1.0)")
    ...
    model = LightGBMBinaryClassifierV1(params=model_config)
    model.fit(frame=train_slice, feature_ids=fids)

    calibrator: IsotonicCalibrator | None = None
    if fit_calibrator:
        cal_x, cal_y = binary_training_frame_to_xy(cal_slice, fids)
        if cal_x.shape[0] >= 2 and len(np.unique(cal_y)) >= 2:
            raw = model.predict_proba_matrix(cal_slice)
            calibrator = IsotonicCalibrator()
            calibrator.fit(raw, cal_y)
